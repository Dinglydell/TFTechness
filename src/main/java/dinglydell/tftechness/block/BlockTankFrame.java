...
public class BlockTankFrame extends Block {
	 ...
	 * 
	 * 
	 * eg. Unfinished Hardened Tank's key = hardened + unfinished = hardenedunfinished */
	...
	public boolean isSideSolid(IBlockAccess paramIBlockAccess, int paramInt1, int paramInt2, int paramInt3,
		...
		if ((paramForgeDirection == ForgeDirection.UP) || (paramForgeDirection == ForgeDirection.DOWN)) {
			return true;
		}
		return false;
	}
	...
	@Override
	public float getExplosionResistance(Entity entity, World world, int x, int y, int z, double paramDouble1,
	...
	public void registerBlockIcons(IIconRegister reg) {
		for (int i = 0; i < BlockTank.Types.values().length; i++) {
			...
			for (int j = 0; j < stLength; j++) {
				...
				String text = textureName + "_" + StringHelper.titleCase(BlockTank.NAMES[i]) + "_"
