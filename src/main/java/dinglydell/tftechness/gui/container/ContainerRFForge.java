...
public class ContainerRFForge extends ContainerTEBase implements ISlotValidator {
	...
	public ContainerRFForge(InventoryPlayer inv, TileRFForge te) {
		super(inv, te);
		for (int i = 0; i < 6; i++) {
			addSlotToContainer(new SlotValidated(this, te, i, 62 + i % 3 * 18, 23 + i / 3 * 23));
		}
		SlotValidatedIngotMold valid = new SlotValidatedIngotMold();
		addSlotToContainer(new SlotValidated(valid, te, TileRFForge.tankBSlotEnd, 131, 61));
		addSlotToContainer(new SlotValidated(valid, te, TileRFForge.tankBSlotEnd + 1, 154, 61));
