...
/**
 * This class and GuiPlanSelectionTech exist to restrict certain anvil plans
 * based on tech
 */
public class GuiPlanButton extends GuiButton {
	public ItemStack item;
	private GuiPlanSelectionTech screen;
	protected static final RenderItem ITEM_RENDERER = new RenderItem();

	public GuiPlanButton(int index, int xPos, int yPos, int width, int height,
			ItemStack ico, GuiPlanSelectionTech gui, String s) {
	...
	public void drawButton(Minecraft mc, int x, int y) {
		if (this.visible) {
			...
			TFC_Core.bindTexture(GuiPlanSelectionTech.texture);
	...
	private boolean isPointInRegion(int mouseX, int mouseY) {
		...
		return mouseX >= xPosition - 1 && mouseX < xPosition + width + 1
				&& mouseY >= yPosition - 1 && mouseY < yPosition + height + 1;
	}
}